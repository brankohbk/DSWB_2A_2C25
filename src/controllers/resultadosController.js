// src/controllers/resultadosController.js
...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    // La carpeta de destino es public/uploads/
    cb(null, path.join(__dirname, '..', '..', 'public', 'uploads'));
  },
  filename: (req, file, cb) => {
    // Generar un nombre único para el archivo: PacienteID-Timestamp-NombreOriginal.ext
    const extension = path.extname(file.originalname);
    // Extraemos el pacienteId del body antes de subir (asumimos que siempre estará)
...
}).single('archivoPDF'); // El nombre del campo en el formulario es 'archivoPDF'
...
export const uploadResultado = (req, res) => {
    upload(req, res, async (err) => {
        ...
        // El archivo se subió correctamente. Ahora registramos el metadata en MongoDB.
        if (!req.file) {
        ...
        if (!pacienteId || !tipoEstudio) {
            // Si faltan datos obligatorios, eliminamos el archivo subido antes de fallar
        ...
        try {
            const newResultado = {
                ...
                nombreArchivo: req.file.filename // Usamos el nombre único generado por Multer
            };
            ...
            // Si falla la DB, también eliminamos el archivo subido
            fs.unlink(req.file.path, () => {
...
export const deleteResultado = async (req, res) => {
    try {
        ...
        const filePath = path.join(__dirname, '..', '..', 'public', 'uploads', resultado.nombreArchivo);
